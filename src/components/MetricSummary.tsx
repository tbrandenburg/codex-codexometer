    ...
    <div className="glass-panel p-4 sm:p-6">
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-brand-100/60">Pulse Check</p>
            <h2 className="text-xl font-semibold text-emerald-50">Community health snapshot</h2>
          </div>
          <span className="badge">Auto QA · 83% confidence</span>
        </div>
        <div className="grid-auto-fit">
          <div className="bg-brand-400/10 rounded-2xl p-4 border border-brand-400/20">
          ...
          <div className="bg-brand-400/10 rounded-2xl p-4 border border-brand-400/20">
            ...
            <p className="text-xs text-brand-100/70 mt-1">Across posts + top comments</p>
          </div>
          <div className="bg-brand-400/10 rounded-2xl p-4 border border-brand-400/20">
            ...
            <p className="text-xs text-brand-100/70 mt-1">Neutral: {neutral.toLocaleString()}</p>
          </div>
          <div className="bg-brand-400/10 rounded-2xl p-4 border border-brand-400/20">
            ...
            <ul className="mt-2 space-y-1 text-sm text-emerald-50">
              {topDays.map((day) => (
                <li key={day} className="flex items-center justify-between">
                  <span>{day}</span>
                  <span className="text-brand-100/70">{metrics.find((m) => m.date === day)?.totalItems ?? 0} items</span>
                </li>
              ))}
            </ul>
          </div>
